...
class Users < Application
  ...
  before :block_if_no_user_by_id?, :only => %w[ :edit :update show ]
...
begin
  ...
  def create
    ...
    if !@user.save
      flash[:warning] = "Problem creating account #{@user.errors.inspect}"
      redirect "/signup"
      # render :signup
      return "Problem creating account #{@user.errors.inspect}"
    end
    session_start(@user)
    
    send_email(nil,nil,
      "",@user.email,
      "Welcome to ImageWiki",
      "Welcome #{@user.login}!"
    );
    
    send_email(nil,nil,nil,"imagewiki@googlegroups.com",
      "ImageWiki Notice: New user: name, email #{@user.login}, #{@user.email}",
  ...
  def destroy
    # TODO disabled til we think it through - prefer to use a flag not a perm deletion
