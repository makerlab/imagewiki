...
void prepare_to_receive_image(const std::string& filename);
void finish_receiving_image();
bool image_part_callback(const MultipartParser::PartInfo& part_info, char *data, size_t data_len);

void prepare_to_receive_label();
void finish_receiving_label();
bool label_part_callback(const MultipartParser::PartInfo& part_info, char *data, size_t data_len);

static FILE *xx_image_file = NULL;
static std::string xx_image_filename("");
static std::string xx_image_label("");
...
void DBServer::handle_add_post (DBServer *server, HTTPServer::Request *request)
{
    ...
    if (!mp_parser.parse_init() || 
        !mp_parser.begin_next_part(&parts_remaining, &part_info)) {
    ...
    // Save the image submission in image.jpg or whatever.
    std::string filename = std::string("/var/tmp/image.") + filename_extension(part_info.filename);
    prepare_to_receive_image(filename);
    while (success && parts_remaining)
    {
        if (success = mp_parser.process_part(part_info, image_part_callback))
        {
            success = mp_parser.begin_next_part(&parts_remaining, &part_info);
        }
    }
    finish_receiving_image();

    // ----------
    // Process the image.
    // ----------
    if (server->m_db->add_image_file(filename, xx_image_label) &&
	server->m_db->save()) {
        std::stringstream htmls;
        htmls << "<html><head><title>Add Image: Succeeded</title></head>"
              << "<body><h1>Add Image: Succeeded</h1>"
              << "<p>Added image with label '" << xx_image_label << "'.</p>"
              << "</body></html>";
        std::string html = htmls.str();
        ResponseStatus(request, 200);
        server->m_server.set_content_type(request, "text/html");
        ...
        htmls << "<html><head><title>Add Image: Failed</title></head>"
              ...
              << "<p>Unable to add image with label '" << xx_image_label << "'.</p>"
              << "</body></html>";
        std::string html = htmls.str();
        ResponseStatus(request, 400);
        server->m_server.set_content_type(request, "text/html");
...
void DBServer::handle_del_post (DBServer *server, HTTPServer::Request *request)
{
    ...
    // Ugh this needs to be changed to accept an object.
    prepare_to_receive_label();
    while (success && parts_remaining) {
      if (success = mp_parser.process_part(part_info, label_part_callback)) {
	success = mp_parser.begin_next_part(&parts_remaining, &part_info);
      }
    }
    finish_receiving_label();

    if (server->m_db->remove_image(xx_image_label) &&
	server->m_db->save()) {
...
// This callback function is called by the multipart form parser to
// handle chunks of image data as they come in.

void prepare_to_receive_image(const std::string& filename)
{
    xx_image_file = NULL;
    xx_image_filename = filename;
    xx_image_label = "";
}
...
void finish_receiving_image()
{
    if (xx_image_file)
    {
        fclose(xx_image_file);
    }
}

bool
image_part_callback(const MultipartParser::PartInfo& part_info, char *data, size_t data_len)
{
    if (xx_image_file == NULL) {
        part_info.dump();
    }
    if (part_info.name == std::string("image")) {
        if (!xx_image_file) {
            xx_image_file = fopen(xx_image_filename.c_str(), "wb");
            if (!xx_image_file) {
                perror("Unable to open image file for writing");
                return false;
            }
        }
        fwrite(data, data_len, 1, xx_image_file);
    }
    else if (part_info.name == std::string("label")) {
        xx_image_label = std::string(data, data_len);
    }
...
bool label_part_callback(const MultipartParser::PartInfo& part_info, char *data, size_t data_len)
{
  if (xx_image_label == std::string("")) {
    part_info.dump();
  }
  if (part_info.name == std::string("label")) {
    xx_image_label = std::string(data, data_len);
  }
...
void DBServer::handle_match_post (DBServer *server, HTTPServer::Request *request)
{
    ...
    // Save the submission in image.jpg or whatever.
    std::string filename = std::string("/var/tmp/image.") + filename_extension(part_info.filename);
    prepare_to_receive_image(filename);
    while (success && parts_remaining)
    {
    ...
    if (!do_match(server->m_db, filename, max_nn_chks, exhaustive_match, &results)) {
        // Badness of some sort.
