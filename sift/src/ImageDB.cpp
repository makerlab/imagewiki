...
{
    for (std::map<unsigned,std::string>::iterator i = m_labels.begin();
         i != m_labels.end();
         i++) {
        if (i->second == label) {
            return true;
...
const std::string& ImageDB::label_for_id(unsigned id)
{
    return m_labels[id];
}
...
bool ImageDB::save(const char* path, bool binary)
{
    ...
    fprintf(file, "%d\n", num_images());
    for (std::map<unsigned,std::string>::const_iterator i = m_labels.begin();
         i != m_labels.end();
         i++) {
